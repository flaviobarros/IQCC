...
- `pchart_limits()`, `pchart_alpha_risk()`, and `cchart.p()` for normal, CF1,
  CF2, and standardized p charts.
- `uchart_limits()`, `uchart_alpha_risk()`, and `cchart.u()` for normal, CF1,
  CF2, and standardized u charts.
- `dsnp_prob_accept()`, `dsnp_arl()`, `dsnp_ass()`, `dsnp_limits()`, and
  `cchart.DSnp()` for double-sampling np design and monitoring.
- `T2.1()`, `T2.2()`, `cchart.T2.1()`, and `cchart.T2.2()` for Hotelling
  T-squared monitoring.
- `gv_stat()`, `gv_limits()`, `gv_alpha_risk()`, and `cchart.GV()` for
  generalized variance monitoring.
...
Use the explicit names `"cf1"` and `"cf2"` in new code. The historical alias
`"CF"` is retained for compatibility, but it maps to `"cf1"` in `cchart.p()`
and to `"cf2"` in `cchart.u()`.

## Example: DS-np chart with automatic limit search

The double-sampling np chart uses two sampling stages to monitor the
nonconforming proportion in high-quality processes. `dsnp_limits()` searches
for feasible fractional limits, and `cchart.DSnp()` builds and plots the chart.
...
upper-limit cases in dimension three. Simulation is available for more general
...
- an auxiliary trace chart for complementary multivariate variability monitoring;
- full DS-np joint optimization over sample sizes and limits;
- generic exact generalized variance quantiles beyond the explicitly supported cases;
- a centralized catalogue of published validation fixtures;
- expanded executable vignettes and replication materials for a future article.
