...
to the normal quantile on the original p scale. For \(z=3\), this becomes
\(4(1-2p)/(3n)\).

The second corrected p chart follows the operational formula in Joekes and
Barbosa (2013). This is important because a generic signed lower-tail
Cornish-Fisher substitution does not reproduce the paper's reported limits.

```{r pchart-example, eval=FALSE}
...
The actual risk is evaluated under the Poisson count model, respecting strict
signaling inequalities.
...
These moments provide the mean, variance, skewness, and kurtosis required by
normal and Cornish-Fisher limits.

```{r gv-example, eval=FALSE}
gv_limits(
  n = 10,
  p = 2,
  det_sigma = 0.5320,
...
When \(|\Sigma|\) is unknown, the package estimates it from the average Phase I
covariance matrix. If \(m\) subgroups are used and
...
This strategy is particularly important for control charts because a plausible
formula may still use a different tail convention, integer threshold, or
parameter-estimation rule than the intended method.
